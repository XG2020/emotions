...
`emotions` 是一个 Nekro Agent 表情包管理插件，提供表情包收藏、语义搜索、分类图库、Web 管理、图床同步和 Agent 工具调用能力。插件使用 Qdrant 向量数据库保存表情语义索引，支持通过自然语言查找合适的表情包。
...
9. 保存配置后，可使用 `xemo_sync status` 检查图床同步状态。
...
13. 保存配置后，可使用 `xemo_sync status` 检查图床同步状态。
...
| 命令 | 说明 |
| --- | --- |
| `xemo_sync status` | 查看同步状态 |
| `xemo_sync upload` | 上传本地图库到图床 |
| `xemo_sync download` | 从图床下载缺失图片 |
| `xemo_sync sync_all` | 双向同步本地和云端 |
| `xemo_sync overwrite_to_remote` | 使用本地图库覆盖云端 |
| `xemo_sync overwrite_from_remote` | 使用云端图库覆盖本地 |
...
| 命令 | 说明 |
| --- | --- |
| `xemo_search <关键词>` | 语义搜索表情包，并展示匹配来源、置信度和图片 |
| `xemo_stats` | 查看表情包总数、向量数量和标签统计 |
| `xemo_list [页码]` | 分页列出已收藏表情包，默认预览图片 |
| `xemo_gallery` / `xemo_categories` | 查看分类及图片数量 |
| `xemo_category_update <分类> <描述>` | 新增或更新分类描述 |
| `xemo_category_clear <分类>` | 清空指定分类图库图片，但保留分类描述 |
| `xemo_sync <任务>` | 执行图床同步任务 |
| `xemo_migrate` | 规范表情元数据路径为 `emotions/` 根目录文件名，并刷新向量 payload |
| `xemo_gallery_check` / `xemo_check` | 验证统一图库状态，检查缺失文件、路径引用和未入库图库图片 |
| `xemo_reindex -y` | 重建全部表情包向量索引 |
...
- `search_emotion`：按文本描述搜索表情包，返回文本候选结果。
...
WebUI 挂载在插件路由下，当前访问路径为：

```text
/plugins/xggm.emotions/
```

如果你的 Nekro Agent 面板地址是 `http://127.0.0.1:8021`，完整访问地址通常为：

```text
http://127.0.0.1:8021/plugins/xggm.emotions/
```
...
6. 修改大量图片或元数据后，可使用 `xemo_reindex -y` 重建索引。
...
- 图床同步涉及远端删除或覆盖任务时，请先使用 `xemo_sync status` 检查状态。
